...
async function executarAcaoAdmin({ action, inst, body, reason, admin, ev }) {
  ...
  switch (action) {
    ...
    case 'confirm-payment': {
      ...
      if (!pay.is_test) throw new Erro(409, 'Pagamentos reais são confirmados pelo gateway, não manualmente.');
      const r = await rpc('bot_confirm_payment', { p_payment: pid, p_actor: admin.id, p_actor_type: 'admin', p_reason: reason || 'Pagamento de teste confirmado pela administração.' });
...
async function adminSalvarConfig(req, admin, body) {
  ...
  for (const [key, value] of Object.entries(body)) {
    ...
    if (['bot_platform_enabled', 'bot_require_approval'].includes(key)) { if (typeof v !== 'boolean') throw new Erro(400, 'Valor inválido.'); }
      ...
      if (v !== 'manual_test') throw new Erro(400, 'Só o modo de teste está disponível até um gateway ser configurado e confirmado.');
    }
