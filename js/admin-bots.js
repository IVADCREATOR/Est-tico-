  ...
  async function carregarWorkers() {
    ...
    if (rc.ok) {
      ...
      $("botSettingsBox").innerHTML = `<form id="botSettingsForm" class="admin-form" novalidate><div class="settings-list">
        ...
        <label>Pagamentos<input value="${s.bot_payment_mode === "manual_test" ? "Modo de teste (confirmação manual)" : esc(s.bot_payment_mode)}" disabled></label>
      </div><p class="field-hint">Nenhum gateway de pagamento real está ligado. Para ativar Mercado Pago, Stripe ou Asaas é preciso confirmar a conta e as credenciais (veja a documentação da plataforma).</p>
      ...
      $("botSettingsForm").onsubmit = (e) => {
        ...
        executar($("botSettingsForm").querySelector('button[type="submit"]'), "admin-settings", {
          ...
          bot_max_instances_per_user: Number($("bsMax").value) || 1, bot_log_retention_days: Number($("bsRet").value)
        });
      };
